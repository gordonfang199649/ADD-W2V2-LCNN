...
def compute_eer(target_scores, nontarget_scores):
    ...
    # 找出最大差異的 index
    # max_diff_index = np.argmax(abs_diffs)
    eer = np.mean((frr[min_index], far[min_index]))
    return eer, frr[min_index], far[min_index], thresholds[min_index]

def compute_frr_far_at_threshold(target_scores, nontarget_scores, threshold):
    """
    計算給定門檻值下的 FRR 和 FAR。

    Args:
        target_scores (np.array): 真實樣本的分數
        nontarget_scores (np.array): 假樣本的分數
        threshold (float): 指定的門檻值

    Returns:
        frr (float): False Rejection Rate
        far (float): False Acceptance Rate
    """
    # FRR: False Rejection Rate = 被拒絕的真實樣本數 / 總的真實樣本數
    false_rejections = np.sum(target_scores < threshold)
    frr = false_rejections / target_scores.size

    # FAR: False Acceptance Rate = 被接受的假樣本數 / 總的假樣本數
    false_acceptances = np.sum(nontarget_scores >= threshold)
    far = false_acceptances / nontarget_scores.size
