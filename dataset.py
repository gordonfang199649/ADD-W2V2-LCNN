...
class RawAudio(Dataset):
    def __init__(self, path_to_features, path_to_meta, meta_file, part, feature='LFCC', feat_len=750, pad_chop=True, padding='repeat'):
        ...
        # 加載 meta.csv
        self.meta_df = pd.read_csv(os.path.join(path_to_meta, part, meta_file)).reset_index(drop=True)
    ...
    def __getitem__(self, idx):
        ...
        label_str = self.meta_df.iloc[idx]['label']
        label = self.label_map[label_str]
        
        # 加載音訊特徵
        filename = "%s.pt" % (filename)
        filepath = os.path.join(self.ptf, self.part, filename)  # 假設特徵是 .pt 格式
        featureTensor = torch.load(filepath)

        # Pad or chop the featureTensor if necessary
        this_feat_len = featureTensor.shape[1]
        if self.pad_chop:
            if this_feat_len > self.feat_len:
                startp = np.random.randint(this_feat_len - self.feat_len)
...
def padding_Tensor(tensor, target_len):
    """
    使用零填充張量以達到目標長度。
    """
    # 取得當前長度和通道數
    ...
    if pad_len <= 0:
        return tensor
    # 使用零填充在右邊
    padding = torch.zeros((tensor.shape[0], pad_len, tensor.shape[2]), dtype=tensor.dtype)
    return torch.cat((tensor, padding), dim=1)


def repeat_padding_Tensor(tensor, target_len):
    """
    使用重複填充張量以達到目標長度。
    """
    current_len = tensor.shape[1]
    if current_len >= target_len:
        return tensor
    repeats = (target_len // current_len) + 1  # 計算重複次數
    # 重複張量並裁剪到目標長度
    repeated_tensor = tensor.repeat(1, repeats, 1)
    return repeated_tensor[:, :target_len, :]


def silence_padding_Tensor(tensor, target_len, silence_value=0.0):
    """
    使用靜音填充（默認為零）張量以達到目標長度。
    """
    current_len = tensor.shape[1]
    pad_len = target_len - current_len
    if pad_len <= 0:
        return tensor
    # 使用靜音值填充在右邊
    padding = torch.full((tensor.shape[0], pad_len, tensor.shape[2]), silence_value, dtype=tensor.dtype)
    return torch.cat((tensor, padding), dim=1)
